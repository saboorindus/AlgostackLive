...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://reddlea.com/api/submitSupportTicket', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        showToast("Support ticket submitted successfully!", "success"); // ✅ show success toast

        setFormData({ name: '', email: '', subject: '', message: '' }); // reset form
      } else {
        const errorText = await response.text();
        console.error('Error submitting ticket:', errorText);
        showToast("Failed to send message. Please try again.", "error"); // show error toast
      }
    } catch (err: unknown) {
      ...
      showToast("An error occurred. Please try again later.", "error"); // show error toast
    }
  };
    ...
    <ToastProvider>
      ...
      {toastVisible && (
        <Toast
          ...
          onOpenChange={(open) => !open && setToastVisible(false)}
        >
